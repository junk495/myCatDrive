...
class MainActivity : AppCompatActivity() {
    ...
    private val navigationReceiver: BroadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent) {
            val data = intent.getParcelableExtraCompat<NavigationData>("navigation_data")
        ...
        override fun onReceive(context: Context?, intent: Intent) {
            when (intent.action) {
                Intents.CONNECTION_UPDATE -> {
                    ...
                    connectedDevice?.let {
                        // FIX: Verwendet jetzt die zentrale Funktion
                        if (PermissionCheck.checkBluetoothPermissions(applicationContext)) {
                            with(mSharedPref.edit()) {
                                putString("last_device_name", PermissionCheck.getDeviceNameSafe(applicationContext, it))
    ...
    private val mDeviceSelectionRequest = registerForActivityResult(
        ...
        if (it.resultCode == RESULT_OK) {
            val device = it.data?.getParcelableExtraCompat<BluetoothDevice>("device")!!
            // FIX: Verwendet jetzt die zentrale Funktion
