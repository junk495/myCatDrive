...
object DurationSerializer : KSerializer<Duration> {
    ...
    override fun serialize(encoder: Encoder, value: Duration) =
        encoder.encodeInt((value.toMillis() / 1000).toInt())

    override fun deserialize(decoder: Decoder): Duration =
        Duration.ofSeconds(decoder.decodeInt().toLong())
}
...
object AnySerializableOnlyValueSerializer : KSerializer<Any?> {
    ...
    override fun serialize(encoder: Encoder, value: Any?) {
        if (value != null)
            encoder.encodeSerializableValue(
                getSerializerForType(value::class.starProjectedType),
                value
            )
        else
        ...
        value.forEach { (k, v) ->
            ...
            if (v != null) {
                composite.encodeSerializableElement(
                    descriptor,
                    index++,
                    getSerializerForType(v::class.starProjectedType),
                    v
                )
            } else {
                composite.encodeSerializableElement(
